...
while true; do
    read -p "Enable Telegram log group topics? (y/n): " BOOL_TELEGRAM_LOGS

    BOOL_TELEGRAM_LOGS=$(echo "$BOOL_TELEGRAM_LOGS" | tr '[:upper:]' '[:lower:]')

    case $BOOL_TELEGRAM_LOGS in
        y)
            BOOL_TELEGRAM_LOGS="True"
            break
            ;;
        n|"")
            BOOL_TELEGRAM_LOGS="False"
            break
            ;;
        *)
            clear
...
if [[ $BOOL_TELEGRAM_LOGS == "True" ]]; then
    read -p "Enter Telegram logs group ID: " TELEGRAM_LOG_GROUP_ID
    clear
    read -p "Enter Telegram log send interval in seconds (default 60): " TELEGRAM_LOG_INTERVAL_SECS
    TELEGRAM_LOG_INTERVAL_SECS=${TELEGRAM_LOG_INTERVAL_SECS:-60}
    clear
    read -p "Enter launch_instance.log topic ID: " TELEGRAM_LOG_TOPIC_LAUNCH_INSTANCE
    clear
    read -p "Enter errors topic ID for ERROR_IN_CONFIG.log and UNHANDLED_ERROR.log: " TELEGRAM_LOG_TOPIC_ERRORS
    clear
else
...
# Telegram Logs Group Topics (optional)
# Create 2 topics in a Telegram forum group and paste their topic IDs below.
TELEGRAM_LOGS_ENABLED=$BOOL_TELEGRAM_LOGS
TELEGRAM_LOG_GROUP_ID=$TELEGRAM_LOG_GROUP_ID
TELEGRAM_LOG_INTERVAL_SECS=$TELEGRAM_LOG_INTERVAL_SECS
TELEGRAM_LOG_TOPIC_LAUNCH_INSTANCE=$TELEGRAM_LOG_TOPIC_LAUNCH_INSTANCE
TELEGRAM_LOG_TOPIC_ERRORS=$TELEGRAM_LOG_TOPIC_ERRORS
EOF
