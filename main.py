...
TELEGRAM_LOGS_ENABLED = os.getenv("TELEGRAM_LOGS_ENABLED", "False").strip().lower() == "true"
TELEGRAM_LOG_GROUP_ID = os.getenv("TELEGRAM_LOG_GROUP_ID", "").strip()
TELEGRAM_LOG_TOPIC_LAUNCH_INSTANCE = os.getenv("TELEGRAM_LOG_TOPIC_LAUNCH_INSTANCE", "").strip()
TELEGRAM_LOG_TOPIC_ERRORS = os.getenv("TELEGRAM_LOG_TOPIC_ERRORS", "").strip()
...
TELEGRAM_LOG_INTERVAL_SECS = get_positive_int_env("TELEGRAM_LOG_INTERVAL_SECS", 60)


def send_telegram_message(message, chat_id=None, topic_id=None):
    if not TELEGRAM_TOKEN:
        return

    target_chat_id = chat_id or TELEGRAM_USER_ID
    if not target_chat_id:
        return

    payload = {"chat_id": target_chat_id, "text": message[:4000]}
    if topic_id:
...
def split_message_for_telegram(message, max_chunk_size=3800):
    ...
    for line in message.splitlines():
        ...
        if len(line_with_newline) > max_chunk_size:
            ...
            for start in range(0, len(line), max_chunk_size):
                chunks.append(line[start:start + max_chunk_size])
            continue

        if current and current_len + len(line_with_newline) > max_chunk_size:
            chunks.append("\n".join(current).strip())
            current = []
            current_len = 0
...
def send_telegram_message_chunks(message, chat_id, topic_id=None):
    for chunk in split_message_for_telegram(message):
        send_telegram_message(chunk, chat_id, topic_id)


def send_telegram_log_message(message, topic_id):
    if not all([TELEGRAM_LOGS_ENABLED, TELEGRAM_LOG_GROUP_ID, topic_id]):
        return

    send_telegram_message_chunks(message, TELEGRAM_LOG_GROUP_ID, topic_id)


class ExcludeLaunchInstanceFilter(logging.Filter):
    def filter(self, record):
        return not record.name.startswith("launch_instance")


class TelegramLogHandler(logging.Handler):
    def __init__(self, chat_id, topic_id, interval_secs):
        super().__init__()
    ...
    def emit(self, record):
        try:
            self.pending_lines.append(self.format(record))
            if (time.time() - self.last_flush) >= self.interval_secs:
                self.flush()
        except Exception:
            self.handleError(record)

    def flush(self):
        if not TELEGRAM_LOGS_ENABLED or not self.pending_lines:
            return

        message = "\n".join(self.pending_lines)
        self.pending_lines = []
        self.last_flush = time.time()
        send_telegram_message_chunks(message, self.chat_id, self.topic_id)
...
def create_telegram_log_handler(topic_id, exclude_launch_instance=False):
    if not all([TELEGRAM_LOGS_ENABLED, TELEGRAM_TOKEN, TELEGRAM_LOG_GROUP_ID, topic_id]):
        return None
    ...
    if exclude_launch_instance:
        handler.addFilter(ExcludeLaunchInstanceFilter())

    return handler


def add_handler_if_configured(logger, handler):
    if handler:
...
try:
    ...
    send_telegram_log_message(str(e), TELEGRAM_LOG_TOPIC_ERRORS)
    print(f"Configuration error. Details saved in ERROR_IN_CONFIG.log: {e}", file=sys.stderr)
    sys.exit(1)
...
logging_step5.propagate = False
fh = logging.FileHandler("launch_instance.log")
fh.setFormatter(logging.Formatter("%(asctime)s - %(levelname)s - %(message)s"))
logging_step5.addHandler(fh)
add_handler_if_configured(
    logging_step5,
    create_telegram_log_handler(TELEGRAM_LOG_TOPIC_LAUNCH_INSTANCE),
)
...
try:
    ...
    notify_msg = f"OCI setup failed before launch attempts started:\n{e}"
    with open("UNHANDLED_ERROR.log", "w", encoding="utf-8") as file:
        file.write(notify_msg)
    send_telegram_log_message(notify_msg, TELEGRAM_LOG_TOPIC_ERRORS)
    print(f"OCI setup error. Details saved in UNHANDLED_ERROR.log: {e}", file=sys.stderr)
    sys.exit(1)
...
def notify_on_failure(failure_msg):
    ...
    send_telegram_log_message(mail_body, TELEGRAM_LOG_TOPIC_ERRORS)
    if NOTIFY_EMAIL:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not Path("UNHANDLED_ERROR.log").exists():
            notify_on_failure(str(e))
